...
class Net(nn.Module):
    ...
    def __init__(self):
        ...
        # 1st fully-connected linear layer 1 with 1024 nodes (sees a 256 x 7 x 7 tensor)
        self.fc1 = lin(in_features=256*7*7, out_features=1024, bias=True, batch_norm=True)
    ...
    def forward(self, x):
        ...
        # Convolutional hidden layer 4 with batch normalization, relu activation function and max pooling
        ...
        # Flatten 256 x 7 x 7 input tensor to first fully conntected layer
        x = x.view(x.size(0), -1)
        
        # Fully connected hidden layer 1 with batch normalization and relu activation function
        x = F.relu(self.fc1(x))
                
        # Dropout layer 5
        x = self.drop(x)
        
        # Fully connected hidden layer 2 with batch normalization and relu activation function
        x = F.relu(self.fc2(x))
        
        # Dropout layer 6
        x = self.drop(x)
        
        # Fully connected hidden layer 3 (no batch normalization, no activation function) => return 
    ...
    def predict(self, x):
        ...
        # Activations with batch normalization of convolutional hidden layer conv4
        a = self.conv4(x)
        activations['conv4'] = a
